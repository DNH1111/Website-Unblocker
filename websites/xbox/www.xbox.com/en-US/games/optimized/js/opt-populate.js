$(document).ready(function() {
    ...
    // END shim code (all for Object.values)
    
      var popTable = (function() {
        var tableheadings = Object.keys(tableContent.headings);
        var bodyrows = Object.values(tableContent.headings[tableheadings[0]]).length;
        regionContent.keyDualgen = $(".translation09").text();
        var translatedHeadings = ["keyGametitle", "keyPublisher", "keyAvailability","keySmartdelivery", "keyDualgen"]
    
        for (var i = 0; i < bodyrows; i++) { $("tbody").append('<tr></tr>'); }
        for (var i = 0; i < tableheadings.length; i++) {
          $("thead tr").append('<th scope="col" class="" colspan="1">' +
                                 '<span class="c-subheading-2">'  + regionContent[translatedHeadings[i]] + '</span>' +
                               '</th>')
          var columnvalues = Object.values(tableContent.headings[tableheadings[i]]);
          for (var k = 0; k < columnvalues.length; k++) {
            var val = columnvalues[k];
            if (columnvalues[k] ===  "Y") {
              val = "<span class='sorttext'>" + "<div class='c-glyph lime-green-c onlyglyph' role='img' aria-label='Available'></div>" + "</span>";
            } else if (columnvalues[k] ===  "N") {
              val = "<span class='sorttext'></span>";
            } else if (columnvalues[k].length ===  0) {
              val = "<span class='sorttext'  style='display: none;'>b</span>";
            } else if (columnvalues[k].indexOf("<url>") !== -1) {
              var text = columnvalues[k].split("<url>")[0];
              var href = columnvalues[k].split("<url>")[1];
              var exclusions = "";
              if (href.indexOf("<exc>") !== -1) {
                exclusions = href.split("<exc>")[1].toLowerCase();
                href = href.split("<exc>")[0];
              }
              if (exclusions.indexOf(urlRegion) === -1) {
                val = '<a class="lime-green-c" href="' + href + '" data-cta="learn">' + text + '</a>';
              } else {
                val = text
              }
            } else if (columnvalues[k] ===  "1") {
              val = "<span class='sorttext'>" + regionContent["keyNowavailable"] + "</span>";
            } else if (columnvalues[k] ===  "2") {
              val = "<span class='sorttext'>" + regionContent["keyComingsoon"] + "</span>";
            } else if (columnvalues[k] ===  "3") {
              val = "<span class='sorttext'>" + regionContent["keyDevelopment"] + "</span>";
            }
            $("tbody tr").eq(k).append('<td>' + val + '</td>');
              
          }
        }
        $("tr").each(function() {
          $(this).find("th:gt(1)").css("text-align", "center");
          $(this).find("td:gt(1)").css("text-align", "center");
        });
        $(".m-banner.pad96").removeClass("pad96");
